...
def file_github_issue(title, body):
    ...
    if not token or not repo_name or token == "your_github_token_here":
        print("GitHub token or repo not configured correctly in .env. Skipping issue creation.")
        return None
    ...
    try:
        ...
        issue = repo.create_issue(title=title, body=body, labels=["backup-failure", "automated-alert"])
